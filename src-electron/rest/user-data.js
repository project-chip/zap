...
const clusterSync = require('../upgrade/cluster-sync')
...
function httpPostShareClusterStatesAcrossEndpoints(db) {
  return async (request, response) => {
    ...
    let syncResult = clusterSync.clusterSync(db, sessionId, endpointTypeIdList)

    return response.status(StatusCodes.OK).json(syncResult)
  }
}
