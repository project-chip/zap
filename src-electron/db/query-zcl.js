...
async function exportAttributeBoundDetails(db, endpointsAndClusters) {
  ...
  SELECT
    ...
    ATOMIC.ATOMIC_SIZE AS ATOMIC_SIZE,
  ...
  AND ATOMIC.ATOMIC_SIZE > 2 AND ENDPOINT_TYPE_ATTRIBUTE.INCLUDED = 1 AND ATT_VALUE IS NOT NULL AND ATT_VALUE != "" AND REPLACE(ATT_VALUE, '0', '')!='x' AND REPLACE(ATT_VALUE, '0', '')!=''
...
WHERE ENDPOINT_TYPE_CLUSTER.CLUSTER_REF in (${endpointTypeClusterRef})
AND ENDPOINT_TYPE_CLUSTER.SIDE = ATTRIBUTE.SIDE AND ENDPOINT_TYPE_CLUSTER.ENABLED=1
AND ATOMIC.ATOMIC_SIZE > 2 AND ENDPOINT_TYPE_ATTRIBUTE.INCLUDED = 1 AND ATT_VALUE IS NOT NULL AND ATT_VALUE != "" AND ENDPOINT_TYPE_ATTRIBUTE.BOUNDED !=0 AND REPLACE(ATT_VALUE, '0', '')!='x' AND REPLACE(ATT_VALUE, '0', '')!=''
GROUP BY CLUSTER.NAME, ATTRIBUTE.NAME
UNION
  SELECT
  ATTRIBUTE.ATTRIBUTE_ID AS ATTRIBUTE_ID,
...
async function exportReportableAttributeDetailsFromEnabledClustersAndEndpoints(
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY CLUSTER.NAME, ENDPOINT_TYPE_CLUSTER.SIDE) CLUSTER_INDEX,
    COUNT (ATTRIBUTE.MANUFACTURER_CODE) OVER () AS MANUFACTURING_SPECIFIC_ATTRIBUTE_COUNT,
    SUM (CASE WHEN ENDPOINT_TYPE_ATTRIBUTE.SINGLETON=1 THEN ATOMIC.ATOMIC_SIZE ELSE 0 end) OVER () AS SINGLETON_ATTRIBUTE_SIZE,
    MAX(ATOMIC.ATOMIC_SIZE) OVER () AS MAX_ATTRIBUTE_SIZE,
...
async function exportClusterDetailsFromEnabledClusters(
  ...
  SELECT
    ...
    SUM(ATOMIC.ATOMIC_SIZE) OVER (PARTITION BY ENDPOINT.ENDPOINT_IDENTIFIER, CLUSTER.NAME, ENDPOINT_TYPE_CLUSTER.SIDE) ATTRIBUTES_SIZE,
...
SELECT * FROM (
  SELECT
    ...
    SUM(ATOMIC.ATOMIC_SIZE) OVER (PARTITION BY ENDPOINT.ENDPOINT_IDENTIFIER) ATTRIBUTES_SIZE,
    ENDPOINT_TYPE.ENDPOINT_TYPE_ID AS ENDPOINT_TYPE_ID,
    ENDPOINT.ENDPOINT_IDENTIFIER AS ENDPOINT_IDENTIFIER,
