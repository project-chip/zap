...
Device-type requirements are scoped to this endpoint type. Without the
ENDPOINT_TYPE_REF join, a device type on another endpoint (for example
Extended Color Light Color Control on endpoint 1) would incorrectly
flag attributes on this endpoint. That was easy to miss in Zigbee,
where cluster configuration is shared across endpoints; in Matter
each endpoint has independent cluster state.
*/
CREATE TRIGGER
...
(
    ...
    AND
      ENDPOINT_TYPE_DEVICE.ENDPOINT_TYPE_REF = ENDPOINT_TYPE_CLUSTER.ENDPOINT_TYPE_REF
    WHERE
