...
async function selectAllEndpointTypes(db, sessionId) {
  ...
  SELECT
    ...
    ENDPOINT_TYPE.NAME,
    ENDPOINT_TYPE.SESSION_PARTITION_REF
  FROM
    ENDPOINT_TYPE
  INNER JOIN
    SESSION_PARTITION
...
/**
 * Returns promise on the details of an endpoint type based on endpoint type id.
 *
 * @param {*} db
 * @param {*} id
...
SELECT * FROM (
  SELECT
    ...
    ENDPOINT_TYPE_DEVICE.ENDPOINT_TYPE_REF AS ENDPOINT_TYPE_ID,
    ...
    ENDPOINT_TYPE_DEVICE.DEVICE_IDENTIFIER AS DEVICE_ID,
    ENDPOINT_TYPE_DEVICE.DEVICE_VERSION AS DEVICE_VERSION,
  ...
  INNER JOIN ENDPOINT_TYPE_DEVICE
  ON ENDPOINT_TYPE_DEVICE.ENDPOINT_TYPE_REF = ENDPOINT_TYPE_CLUSTER.ENDPOINT_TYPE_REF
  INNER JOIN ENDPOINT
  ON ENDPOINT_TYPE_DEVICE.ENDPOINT_TYPE_REF = ENDPOINT.ENDPOINT_TYPE_REF
