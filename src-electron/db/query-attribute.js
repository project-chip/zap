...
async function selectAttributeDetailsFromEnabledClusters(
  ...
  GROUP BY CLUSTER.MANUFACTURER_CODE, CLUSTER.NAME, ATTRIBUTE.NAME, ATTRIBUTE.SIDE
  ORDER BY CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ENDPOINT_TYPE_CLUSTER.SIDE, ATTRIBUTE.CODE
        `
    )
    .then((rows) => rows.map(mapFunction))
}
...
async function selectReportableAttributeDetailsFromEnabledClustersAndEndpoints(
  ...
  GROUP BY ENDPOINT.ENDPOINT_IDENTIFIER, CLUSTER.MANUFACTURER_CODE, CLUSTER.NAME, ATTRIBUTE.NAME, ATTRIBUTE.SIDE
  ORDER BY ENDPOINT.ENDPOINT_IDENTIFIER, CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ATTRIBUTE.CODE
        `
    )
    .then((rows) => rows.map(mapFunction))
}
