...
async function selectAttributeDetailsFromEnabledClusters(
  ...
  SELECT
    ...
    CASE
      WHEN ATOMIC.IS_STRING=1 THEN 
        ...
        ELSE ATOMIC.ATOMIC_SIZE
    END AS ATOMIC_SIZE,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ENDPOINT_TYPE_CLUSTER.SIDE ORDER BY CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ATTRIBUTE.MANUFACTURER_CODE, ATTRIBUTE.CODE) CLUSTER_INDEX,
  ...
  ORDER BY CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ENDPOINT_TYPE_CLUSTER.SIDE, ATTRIBUTE.MANUFACTURER_CODE, ATTRIBUTE.CODE
        `
    )
    .then((rows) => rows.map(mapFunction))
}
...
async function selectReportableAttributeDetailsFromEnabledClustersAndEndpoints(
  ...
  GROUP BY ENDPOINT.ENDPOINT_IDENTIFIER, CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ATTRIBUTE.MANUFACTURER_CODE, ATTRIBUTE.CODE, ATTRIBUTE.SIDE
  ORDER BY ENDPOINT.ENDPOINT_IDENTIFIER, CLUSTER.MANUFACTURER_CODE, CLUSTER.CODE, ATTRIBUTE.MANUFACTURER_CODE, ATTRIBUTE.CODE
        `
    )
    .then((rows) => rows.map(mapFunction))
}
