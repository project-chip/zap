...
ORDER BY ENDPOINT_TYPE.NAME, DEVICE_TYPE_CODE, DEVICE_TYPE_PROFILE_ID`,
      [sessionId]
    )
    .then((rows) => rows.map(mapFunction))
}
...
async function exportCliCommandCountFromEndpointTypeCluster(
  ...
  WHERE ENDPOINT_TYPE_CLUSTER.ENDPOINT_TYPE_REF IN (${endpointTypeIds}) AND ENDPOINT_TYPE_CLUSTER.ENDPOINT_TYPE_CLUSTER_ID = ?
        `,
      [endpointClusterId]
    )
