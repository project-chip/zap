 ...
 */
async function processReloadDeviceTypes(db, packageId, data) {
  let deviceTypes = data.map((x) => prepareDeviceType(x))
  return queryLoader.reloadDeviceTypes(db, packageId, deviceTypes)
}
...
async function loadIndividualSilabsFile(db, filePath, sessionId) {
  try {
    ...
    querySessionNotification.setNotification(
      db,
      'ERROR',
      `Error reading xml file: ${filePath}, Error Message: ` + err,
      sessionId,
      1,
      0
    )
