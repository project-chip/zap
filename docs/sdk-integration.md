...
| templates | Array of JS objects containing keys: _path_, _name_, _output_, _iterator_         | Lists the individual files. Path should be relative against the location of the `gen-template.json` itself, output is the name of the generated file, and name is a human readable name that might show in the UI. The key "iterator" is optional and can be used in case where the output file is not a single file, but a multitude of files, created for each item being iterated over. For example, if the value of iterator is "availableClusters", then the generator will iterate over each available cluster in the database, and then generatea a file for each one, populating the initial template context with the cluster information. In thi case, the _path_ key should contain the `{key}` patterns, where this pattern will be replaced with the value of the `key` attribute of the iterative object, cluster in this example.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
