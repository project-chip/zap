...
ZAP file extensions allow users to extend the functionality of existing ZAP configuration files(.zap) by merging additional data from extension files(.zapExtension). This feature is particularly useful for adding new clusters, attributes, or other configuration elements without modifying the base ZAP file.
...
2. **Extension ZAP File(.zapExtension)**: A supplementary file that adds to elements in the base file. eg [zapFileExtension](../test/resource/zapExtension1.zapExtension)
3. **Merging Process**: During the import process, the extension file's content is merged into the base file content. However if you save this imported file then the export collates the data between the 2 files.
