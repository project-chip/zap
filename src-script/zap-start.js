  ...
  .then(() => {
    ...
    if (process.platform == 'linux' && executable == 'electron') {
      if (!process.env.DISPLAY) {
        console.log(`
${emojiUtil.formatMessage('⛔', 'You are on Linux and you are attempting to run zap in UI mode without DISPLAY set.')}
${emojiUtil.formatMessage('⛔', 'Please set your DISPLAY environment variable or run zap-start.js with a command that does not require DISPLAY.')}`)
        process.exit(1)
      }
    }
